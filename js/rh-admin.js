...
function saveStaff(){
  ...
  if(editingStaffIndex !== null){
    ...
    staff.push({ id: DB.uid('CPI-STF'), statut:'Actif', ...fields });
    DB.set(DB.KEYS.staffAdmin, staff);
...
function genererPaieMois(){
  ...
  const total = staff.reduce((s,x)=> s + Number(x.salaireFixe||0), 0);
  ...
  staff.forEach(s=>{
    payroll.push({
      ...
      poste: s.poste, mois: curMonth, salaireNet: s.salaireFixe,
  ...
  DB.logActivity('<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-hand-coins-icon lucide-hand-coins"><path d="M11 15h2a2 2 0 1 0 0-4h-3c-.6 0-1.1.2-1.4.6L3 17"/><path d="m7 21 1.6-1.4c.3-.4.8-.6 1.4-.6h4c1.1 0 2.1-.4 2.8-1.2l4.6-4.4a2 2 0 0 0-2.75-2.91l-4.2 3.9"/><path d="m2 16 6 6"/><circle cx="16" cy="9" r="2.9"/><circle cx="6" cy="5" r="3"/></svg>', `Génération de la paie du personnel administratif pour ${curMonth} — ${DB.fmtFCFA(total)} débités de la caisse`);
  toast(`✔ Paie générée pour ${staff.length} membres — ${DB.fmtFCFA(total)} débités. Nouveau solde : ${DB.fmtFCFA(DB.soldeCaisse())}`);
  renderAll();
}
