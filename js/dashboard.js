...
function renderDashboard(withEntryAnim){
  ...
  document.getElementById('statDebiteurs').textContent = debiteurs.length;
  ...
  if(debiteurs.length === 0){
    ...
    body.innerHTML = debiteurs.map(e=>{
      const el = students.find(s=>s.id===e.eleveId) || {nom:'?',prenom:'?'};
      return `<tr>
  ...
  // Alert header count
  document.querySelector('#alertHeader .alert-box-title').textContent = `ALERTES PAIEMENT EN RETARD (${debiteurs.length})`;
